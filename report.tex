...
\subsubsection{Node}
In ns-3 devices like host and end-user are represented as abstractions of nodes by the class Node. This class provides management of how these devices will be represented in simulations. 

\subsubsection{Application}
Application is abstractions of user program that generates some activity to be simulated which is represented by the class Application. This class provides methods for managing the representations of applications in simulations.
...
\end{lstlisting}
\end{footnotesize}
In order to maintain the container of sockets, some facilities are provided to the application. It is necessary for the application to add or remove sockets in the container. And it might also need possibility to merge containers.   
...
graphical overview of this structure can be seen in figure \vref{fig_overview}.
...
together with both a CMU and CSU. To support these scenarios we have added 
senderUnit and receiverUnit objects to the class. These objects are derived from 
the CacheStoreUnit class so their purpose is to modify packets. The main idea of 
CacheCast is to intercept the packets before they are transmitted onto the link and 
remove redundant payload. In ns-3 this interception is done in the 
CacheCastNetDevice::TransmitStart() function.  The packets need to be 
intercepted also on the receiver side, and this is done in 
CacheCastNetDevice::Receive. In these functions the HandlePacket() function of 
the senderUnit and receiverUnit is called, respectively. 


\subsubsection{CacheCastChannel}
The CacheCastChannel is no more than an adapted version of the 
PointToPointChannel is order to support the CacheCastNetDevice. No packet 
handling related to the CacheCast technique is done in this class.
...
\subsubsection{Underlying packet handling mechanism}
