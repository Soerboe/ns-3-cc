...
The ns-3 simulator\cite{ns-3} is a discrete-event network simulator, in which the simulation core and models are implemented in C++. 
ns-3 simulator is an open-source project, intended for educational and 
scientific use. Main purpose for usage of this tool is to simulate  different routing protocols, scenarios that occurs in different
networks. These scenarios and designs of networks are implemented by user.
ns-3 isbuilt as a library which may be statically or dynamically linked to a C++ main program that defines the simulation
topology and starts the simulator. ns-3 also exports its API to Python, allowing Python programs to import
modules same way as the ns-3 library is linked by executables in C++.

ns-2 is a version prior to ns-3, and uses an API that is not supported by ns-3.
Some models from ns-2 have already been ported from ns-2 to ns-3, and some has not yet 
been implemented but it supports nearly all popular network protocols used nowadays. 
