...
The ns-3 simulator\cite{ns-3} is a discrete-event network simulator. It is an 
open-source project written in C++ and is intended for educational and 
scientific use. Main purpose for usage of this tool is to simulate different 
scenarios that accure in network. These scenarios and designs of network are 
implemented by user. ns-2 is a version prior to ns-3, which is a new simulator 
that does not support the ns-2 APIs. Some models from ns-2 have already been ported 
from ns-2 to ns-3, and some has not yet been implemented in ns-3. Version of ns-3 
we have used is ns-3.13. 
...
In CacheCache, applications stands for creating and binding sockets. CacheCast API
...
Packet is sent to all socket in the socket container by using function Msend. This function 
return a bool depending on if packet was delivered succesful to all sockets in the container. 
There is no possibility for applications to know which one of socket did failed during the sending 
...
CMU and CSU. First, CacheCastNetDevices are created for each of these nodes.
