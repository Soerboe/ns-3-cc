...
According to cachecast design, CacheStoreUnit is responsible for caching the payload from the 
...
In our implementation CacheCastNetDevice receives the packet in CacheCastNetDevice::Receive 
function. After determining that it is a CacheCastPacket that has arrived, the packet is handled 
to Receiver Unit's (abstracted by CSU) packethandler.

It is important to assure that there are enough slots in the cache 
so the payload of the packet can be stored in a correct manner. Currently we have not implemented a
solution for scenario where the CacheCast packet is too large for CSU to handle. (Therefore it 
chooses to reject the packet if that is the case.)

When the packet train arrives at CSU, it knows that only the first header of a packet train 
is followed by payload data. 
For every header that is received by the packethandler of CSU, it checks size of payload to 
determine where the packet is complete or is it just a header. If payload size of the packet is
0, packethandler knows that only header has arrived and copies payload chunks from the cache 
store slots to the packet buffer. On arrival of a complete packet, the packethandler knows that
this packet is not stored in the cache and stores the payload chunks from the incoming packet.
...
mapped struct is a combination of the packet IP address and the payload ID, and is used to avoid 
duplicated payload chunks in cache storage. Implementation of Handlepacket() in CacheStoreUnit
is show in listning \ref{impl_csu_oslo}


\begin{footnotesize}
% -------------- WIDTH ---------------------------]
\begin{lstlisting}[language=C,caption = Implementation of Handlepacket() in CacheStoreUnit, label = impl_csu_oslo, frame=tb]
bool CacheStoreUnit::HandlePacket (Ptr<Packet> p){
  NS_LOG_FUNCTION (p);
  NS_ASSERT_MSG (m_size > 0, 
  "CSU's cache size must be a positive integer");
  NS_ASSERT_MSG (m_slotSize > 0, 
  "CSU's slot size must be a positive integer");
