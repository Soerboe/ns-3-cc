...
the payload size. These are necessary for the CMU and server unit in order to 
...
packets has been processed by a router node. There is no problem with this 
design choice since packet tags do not affect how the packets it being 
transmitted on a link, and because the payload ID and payload size is the same 
in the whole lifetime of a CacheCast packet. The CacheCastTag is added to the 
packets by the \textsf{CacheCast::Msend()} function (see section 
\ref{api_impl}).
